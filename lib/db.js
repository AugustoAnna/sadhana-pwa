...
(function () {
  ...
  function importAll(data) {
    ...
    emit({ store: '*' });
    const done = chain.then(() => conn && transaction('readwrite', (kvStore, entryStore) => {
      kvStore.clear();
      entryStore.clear();
      kv.forEach((r) => kvStore.put(r));
      entries.forEach((r) => entryStore.put(r));
    }));
    // The caller sees a failure, but the write chain must stay usable: a rejected chain would
